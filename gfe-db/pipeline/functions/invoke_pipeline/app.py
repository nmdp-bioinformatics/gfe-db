...
def lambda_handler(event, context):
    ...
    try:
        if "releases" in event:
            ...
            if not all(
                [
                    isinstance(event[arg], bool)
                    for arg in execution_input_bool_keys
                    if arg in event
                ]
            ):
                raise ValueError(
                    f'{", ".join(execution_input_bool_keys)} must be boolean values'
                )
            ...
            if not all(
                [
                    is_valid_release(release, release_pattern)
                    for release in event["releases"].split(",")
                ]
            ):
                raise ValueError(
                    f"releases must contains strings that match {release_pattern}"
                )
            ...
            new_releases, params = parse_state(
                branches_state_path, pipeline_params_path
            )
        ...
        if new_releases:
            ...
            for release in new_releases:
                ...
                logger.info(
                    f"Running pipeline with these parameters:\n{json.dumps(params_input)}"
                )
            ...
            return {"status": 200, "message": "No new releases found"}
...
def write_config(path):
    ...
    try:
        ...
        if response["ResponseMetadata"]["HTTPStatusCode"] == 200:
            ...
            logger.error(
                f'Failed to write config file to {path}. HTTPStatusCode: {response["ResponseMetadata"]["HTTPStatusCode"]}'
            )
            return
...
def read_config(path):
    ...
    try:
        response = s3.get_object(
            Bucket=DATA_BUCKET_NAME, Key="/".join(path.split("/")[3:])
        )
        ...
        if response["ResponseMetadata"]["HTTPStatusCode"] == 200:
            ...
            logger.error(
                f'Failed to read config file to {path}. HTTPStatusCode: {response["ResponseMetadata"]["HTTPStatusCode"]}'
            )
            return
...
def check_new_releases(previous_state, current_state):
    ...
    if branches_added:
        logger.info(
            f"Found {new_branches_count} new branches: {json.dumps(current_state[-new_branches_count:])}"
        )
        ...
        new_releases = sorted(
            [
                int(release)
                for release in list(set(current_state).difference(previous_state))
            ]
        )
        ...
        elementwise_difference = list(
            set(
                np.diff(
                    [release for release in previous_state_last_release + new_releases]
                )
            )
        )
        new_branches_are_valid_releases = (
            len(elementwise_difference) == 1 and elementwise_difference[0] == 10
        )
...
def check_current_executions(state_machine_arn):
    ...
    for executions_arn in executions_arns:
        response = sfn.describe_execution(executionArn=executions_arn)

        releases_processing = releases_processing + [
            params["releases"] for params in json.loads(response["input"])
        ]
