...
def lambda_handler(event, context):
    ...
    event["file_paths"] = [f's3://{data_bucket_name}/{path}' for path in output_file_paths]
    return event

# Schema map
output_headers = {
    "all_cds": ['gfe_name', 'bp_seq_id', 'bp_sequence', 'aa_seq_id', 'aa_sequence'],
    "all_features": ['accession', 'hash_code', 'locus', 'rank', 'sequence', 'term', 'gfe_name', 'allele_id', 'hla_name', 'imgt_release'],
    "all_groups": ['gfe_name', 'allele_id', 'hla_name', 'ard_id', 'ard_name', 'locus', 'imgt_release'],
