...
CREATE INDEX seq_index IF NOT EXISTS FOR (seq:Sequence) ON (seq.seq_id); // Constraint slows down loading
...
CREATE INDEX g_index IF NOT EXISTS FOR (_g:G) ON (_g.ard_id);
CREATE INDEX lg_index IF NOT EXISTS FOR (_lg:G) ON (_lg.ard_id);
CREATE INDEX lgx_index IF NOT EXISTS FOR (_lgx:G) ON (_lgx.ard_id);

// CREATE CONSTRAINT cds_constraint IF NOT EXISTS ON (cds:CDS) ASSERT cds.seq_id IS UNIQUE; // Not sure if this is unique
CREATE INDEX cds_index IF NOT EXISTS FOR (cds:CDS) ON (cds.gfe_name, cds.seq_id);
